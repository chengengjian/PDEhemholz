...
void smoothMesh(CHarmonicMapMesh* pMesh) {
    for (int i = 0; i < 1000; i++) {
        for (CHarmonicMapMesh::MeshVertexIterator viter(pMesh); !viter.end(); viter++) {
            CHarmonicMapMesh::CVertex* pV = *viter;
            if (pV->boundary()) {
                continue;
            }
            CPoint p = pV->point();
            ...
            double z = 0;
            int count = 0;
            for (CHarmonicMapMesh::VertexVertexIterator vviter(pV); !vviter.end(); vviter++) {
                CHarmonicMapMesh::CVertex* pv = *vviter;
                x += pv->point()[0];
                y += pv->point()[1];
                z += pv->point()[2];
                count++;
